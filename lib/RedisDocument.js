...
var util            = require('util')
  , async           = require('async')
  , getDeepProperty = require(
                        '../node_modules/modinha/lib/initialize'
                      ).getDeepProperty
  ;
...
var getDeepProperty = require('../node_modules/modinha/lib/initialize').getDeepProperty;
  ...
  Model.__indices.forEach(function (config) {
    ...
    if (config.type === 'hash') {
      var key   = (Array.isArray(config.key))
        ...
        , value = getDeepProperty(data, config.value.split('.'))
        ;
...
RedisDocument.reindex = function (multi, data, orig) {
  ...
  Model.__indices.forEach(function (config) {
    ...
    if (config.type === 'hash') {
      var key      = config.key
        , newField = (Array.isArray(config.field))
                      ? Model.indexKey(config.field, data)
                      : data[config.field]
        , oldField = (Array.isArray(config.field))
                      ? Model.indexKey(config.field, orig)
                      : orig[config.field]
        , newValue = data[config.value]
        , oldValue = orig[config.value]
        , changed  = (newField !== oldField
                   || newValue !== oldValue)
                   ;
...
RedisDocument.intersects = function (collection, unique) {
  var Model       = this
    , lcollection = Model.collection
    , rcollection = collection
    , ccollection = rcollection.charAt(0).toUpperCase()
                  + rcollection.slice(1)
    , lunique     = Model.uniqueId
    , runique     = '_id'
    ;
  ...
  Model['add' + ccollection] = function (lobject, robject, callback) {
    var multi = Model.__client.multi()
      , lid = (typeof lobject === 'object') ? lobject[lunique] : lobject
      , rid = (typeof robject === 'object') ? robject[runique] : robject
      , lkey = lcollection + ':' + lid + ':' + rcollection
      , rkey = rcollection + ':' + rid + ':' + lcollection
      , score = Date.now()
      , lscore = lobject.created || score
      , rscore = robject.created || score
      ;
