...
RedisDocument.intersects = function (collection, unique) {
  ...
  Model['add' + ccollection] = function (lobject, robject, callback) {
    var multi = Model.__client.multi()
      , lkey = lcollection + ':' + lobject[lunique] + ':' + rcollection
      , rkey = rcollection + ':' + robject[runique] + ':' + lcollection
      , lscore = lobject.created
      , rscore = robject.created
      ;

    multi.zadd(lkey, rscore, robject[runique]);
    multi.zadd(rkey, lscore, lobject[lunique]);
    console.log('ABOUT TO ADD:', multi.queue);
    multi.exec(function (err, result) {
      if (err) { return callback(err); }
  ...
  Model['remove' + ccollection] = function (lobject, robject, callback) {
    ...
    multi.zrem(lkey, robject[runique]);
    multi.zrem(rkey, lobject[lunique]);
    multi.exec(function (err, result) {
      if (err) { return callback(err); }
      callback(null, result);
    });
  ...
  Model['listBy' + ccollection] = function (robject, options, callback) {
    ...
    options.index = rcollection + ':' + robject[runique] + ':' + lcollection;
